...
const MODE_CONFIG: Record<ModeKey, ModeConfig> = {
  ...
  track: { label: 'Track Shot', tag: 'TRK', color: '#e879f9', glow: 'rgba(232,121,249,0.4)', accentRgb: '232,121,249', multiplier: 1, maxConcurrent: 1 },
};

const MODE_ORDER: ModeKey[] = ['flick', 'grid', 'track'];
...
const DIFF_GRADE_BONUS: Record<ModeKey, number> = { flick: 0, grid: 0, track: 0 };
...
function calcGrade(accuracy: number, avgReaction: number, diff: ModeKey | string): Grade {
  if(typeof diff === 'string' && diff !== 'flick' && diff !== 'grid' && diff !== 'track') diff = 'flick';
  const adj = accuracy + DIFF_GRADE_BONUS[diff as ModeKey];
...
export default function AimTrainerPage() {
  ...
  // Config
  const [difficulty, setDifficulty] = useState<ModeKey>('flick');
  ...
  const configRef = useRef<ModeConfig>(MODE_CONFIG.flick);
  const sessionDurationRef = useRef<number | null>(10);
  const targetsCountRef = useRef(0);
  const difficultyAtStartRef = useRef<ModeKey>('flick');
  ...
  const spawnTarget = useCallback(() => {
    ...
    const mode = difficultyAtStartRef.current;
    
    let x = 0, y = 0;
    const size = 64; // fixed size for aimlab style
    const padding = size / 2 + 8;
    
    let gridCol, gridRow;
    
    if (mode === 'flick') {
      x = padding + Math.random() * Math.max(0, rect.width - padding * 2);
      y = padding + Math.random() * Math.max(0, rect.height - padding * 2);
    } else if (mode === 'grid') {
      const cols = 4;
      const rows = 4;
      const gridWidth = Math.min(rect.width * 0.8, 600);
      const gridHeight = Math.min(rect.height * 0.8, 400);
      const cellW = gridWidth / cols;
      const cellH = gridHeight / rows;
      const startX = (rect.width - gridWidth) / 2;
      const startY = (rect.height - gridHeight) / 2;
      
      const occupied = new Set(targetsCountRef.current > 0 ? (areaRef.current as any)._activeGridCells || [] : []);
      const available = [];
      ...
      const choice = available[Math.floor(Math.random() * available.length)] || {c:0, r:0};
      x = startX + choice.c * cellW + cellW/2;
      y = startY + choice.r * cellH + cellH/2;
    ...
    if (mode === 'track') {
      const angle = Math.random() * Math.PI * 2;
      const speed = 250 + Math.random() * 150; // Random speed between 250 and 400
      vx = Math.cos(angle) * speed;
      vy = Math.sin(angle) * speed;
    }

    const target: TargetT = { id, x, y, size, points, spawnTime: Date.now(), lifetime: 999999, vx, vy };
    
    if (mode === 'grid') {
      target.gridCol = gridCol;
      target.gridRow = gridRow;
      const act = (areaRef.current as any)._activeGridCells || [];
      act.push(gridCol + ',' + gridRow);
      (areaRef.current as any)._activeGridCells = act;
    }
  ...
  const beginCountdown = useCallback(() => {
    ...
    setLastResult(null);
    const cfg = MODE_CONFIG[difficulty];
  ...
  const hitTarget = useCallback((target: TargetT, e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (target.gridCol !== undefined) {
      if (areaRef.current) {
         let act = (areaRef.current as any)._activeGridCells || [];
         act = act.filter((v: string) => v !== target.gridCol + ',' + target.gridRow);
         (areaRef.current as any)._activeGridCells = act;
      }
    }
    ...
    targetsCountRef.current -= 1; // Manually decrement so spawnTarget works immediately
  ...
  const activeCfg = (phase === 'running' || phase === 'paused' || phase === 'countdown') ? configRef.current : MODE_CONFIG[difficulty];
            ...
            {MODE_ORDER.map(key => {
              ...
              return (
                <button
                  ...
                  onMouseEnter={() => setHoveredDiff(key as ModeKey)}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: `rgba(${MODE_CONFIG[lastResult.difficulty as ModeKey]?.accentRgb || '255,255,255'},0.12)`,
                  border: `1px solid rgba(${MODE_CONFIG[lastResult.difficulty as ModeKey]?.accentRgb || '255,255,255'},0.35)`,
                ...
                <div style={{ maxHeight: '260px', overflowY: 'auto', padding: '0.5rem 0.75rem 0.75rem' }}>
                  {history.slice(0, 20).map(s => (
                    <div key={s.id} style={{
                      ...
                      <span style={{ color: MODE_CONFIG[s.difficulty as ModeKey]?.color || '#ffffff', fontWeight: 700, fontSize: '0.68rem' }}>
                        {MODE_CONFIG[s.difficulty as ModeKey]?.label || s.difficulty}
                      </span>
